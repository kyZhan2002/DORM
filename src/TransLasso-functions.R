...
agg.fun<- function(B, X.test,y.test, total.step=10, selection=F){
  ...
  if(selection){#select beta.hat with smallest prediction error
    ...
    term1 = -colSums((y.test-X.test%*%B)^2)/2
    theta.hat<- exp(term1 - median(term1))
    ...
    for(ss in 1:total.step){
      #cat('step',ss,beta,"\n")
      term1 <- -colSums((y.test - X.test %*% B)^2) / 2
      term2 <- colSums((as.vector(X.test %*% beta) - X.test %*% B)^2) / 8
      
      #cat("term1",term1,"\n")
      #cat('term2',term2,"\n")
      
      #if(min(term1) <= -800){break}
      #if(sum(term2) >= 100){break}
      theta.hat <- exp(term1 + term2 -median(term1))
      ...
      theta.hat[abs(theta.hat) < 1e-10] <- 0
      #cat("theta.hat",theta.hat,"\n")
      #cat("theta.old",theta.old,"\n")
      #print(sum(abs(theta.hat - theta.old)))
      flag = (sum(abs(theta.hat - theta.old)) <= 0.06)
      #print(flag)
