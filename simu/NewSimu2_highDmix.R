...
Nlist = rep(2000,5)      # Source sample size of X. 
nlist = rep(500,5)      # Source sample size of Y.
n0 = 2000                # Target sample size of X0.
...
q=4
Data = Generate_Simulation_Data(L,p,q,Nlist,nlist,n0,mixture,delta,trues,MU_A,BETA,ptemp,
...
for (i in 1:(ntar_num)) {
  ntar = ntar_arr[i]
  TL = mytranslasso(Data$X0,Data$Xlist,Data$Y0,Data$Ylist,ntar,q)
...
for(i in 1:nsmax){
  result = get_REMIX_beta(Data$Xlist,Data$Xtrainlist,Data$Ylist,Data$Ytrainlist,Data$X0,Data$X0train,nlist,q,smax_array[i],
                          penalty = TRUE,alpha = 0.5,rho_pseudo = FALSE, dr_type = "rf")
